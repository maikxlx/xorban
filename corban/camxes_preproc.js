...
var diares_vowels = "äëïöü";
var breve_vowels =  "ăĕĭŏŭ";
...
function camxes_preprocessing(input) {
	...
	input = input.replace(/ë/gm,"aaa");
	input = input.replace(/ö/gm,"aaa");
	input = input.replace(/y/gm,"aaa");
	return input;
}
...
function check_for_diacritic_vowel(chr) {
	...
	for (i = 0; i < 5; i++)
		if (diares_vowels[i] == chr) return [plain_vowels[i], "\""];
	for (i = 0; i < 5; i++)
		if (breve_vowels[i] == chr) return [plain_vowels[i], "°"];
	for (i = 0; i < 5; i++)
