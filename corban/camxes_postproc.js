...
function camxes_postprocessing(text, mode) {
	...
	text = text.replace(/(gismu|lujvo|fuhivla|cmene|cmevla),([A-Za-z0-9]+)/g, "$1:$2");
	...
	if (!with_selmaho) text = text.replace(/[A-Za-z0-9_']+:/g, "");
	text = text.replace(/\]\[/g, "] [");
	text = prettify_brackets(text);
	text = text.replace(/ +/gm, " ");
	text = text.replace(/^ +/, "");
	text = toaq_tone_marks_to_diacritics(text);
	text = text.replace(/aaa/gm, "ë");
	return text;
}
...
function delete_superfluous_brackets(string) {
	...
	**/
	do {
		...
		var j = i;
		while (str[i].search(/[A-Za-z0-9_':~\-\^\"\?\\\/°]/) == 0 && i < str.length) i++;
		while (str[i] == ']') {
			if (j <= 0) {
